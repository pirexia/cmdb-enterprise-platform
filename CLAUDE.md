...
- Current release: **v3.5.11** — see "Plan Activo" § Releases recientes below for full detail on this and all prior releases (this line is a pointer only; kept brief to avoid drifting out of sync with the authoritative section).
...
**Versión actual en producción:** v3.6.0 (tag creado, release GitHub, main/develop sincronizados, 2026-07-31)
**Rama activa:** `develop`
**PRs abiertos:** ninguno
**Issues abiertos para la siguiente sesión:** #153 (npm audit exceljs→uuid), #152 (otplib v12→v13, auth-crítico). Follow-ups surgidos durante #172 (no abiertos como issues todavía — ver `docs/internal/plans/2026-07-16-open-issues-remediation-roadmap.md`, ahora fuera de origin): `index.ts` no es importable/montable en tests (app.listen() incondicional), Task 6 bulk-import batch-create con bucle de hasta 500 INSERTs secuenciales dentro de una transacción, `backend/src/modules/internal/alerts.ts:117-121` con el mismo patrón mutación-sin-transacción que #172 corrigió en todo lo demás. **Nuevo (surgido durante #191):** extracción de la administración de usuarios (`GET /api/users`, role, status, reset-password, change-password, erase GDPR) de `index.ts` a `modules/users/` propuesta como candidata a versión futura (v3.6.0 se usó para el conector Greenbone en su lugar, ver más abajo) — desbloquearía tests supertest reales contra el router (el bug de #191 no lo cazó ningún test porque toda la suite mockea `$executeRaw`); auth/SSO/MFA se queda en el legacy por ahora (alto acoplamiento a JWT/cookies/estado SSO). Nota reforzada por v3.6.0: el mismo patrón de brecha ("ningún test toca Postgres real") volvió a morder en `PATCH /api/vulnerabilities` — ver la entrada de v3.6.0.

### Issue #191 — resuelto y desplegado en producción (2026-07-23/24)

`PATCH /api/users/:id/role` devolvía **500** al cambiar el rol de cualquier usuario (detectado promocionando un usuario AD recién creado como VIEWER, pero **no específico de AD** — afectaba a todos). Causa raíz: `UPDATE "users" SET role = ${role}` en `index.ts:1189` vinculaba `role` como parámetro Prisma `text` contra la columna enum de Postgres `users.role` (`"UserRole"`), sin cast implícito text→enum (`42804`/`P2010`). Reproducido de forma aislada contra Postgres real (bind exacto de Prisma) y confirmado contra el error real en logs de producción. Fix: cast explícito `::"UserRole"`, coherente con el `::uuid` ya presente en la misma sentencia; `role` ya validado contra la allowlist ADMIN/AUDITOR/VIEWER antes de esa línea. Barrido completo del repo confirma que es la **única** asignación raw a una columna enum (`status` de bulk-import es `varchar`, `active` es `boolean`). No es regresión de #172 (v3.5.6): el patrón existe desde el commit original del panel de usuarios; #172 solo lo envolvió en `$transaction`. `userAdminAuditTransaction.test.ts` no lo detectó porque mockea `$executeRaw` — documentado explícitamente en el test que ningún test de este repo toca Postgres real; verificación autoritativa fue el bind aislado contra la BD + verificación del `dist` compilado. 477/477 tests, `tsc --noEmit` limpio. PR #192 (fix→develop) + PR #193 (develop→main), tag v3.5.8, release GitHub.
...
- **v3.6.0** ✅ LIBERADA (tag `v3.6.0`, PR #209 feature→develop + PR #210 develop→main, release en GitHub, main/develop sincronizados, 2026-07-31): reescritura completa del conector de importación de vulnerabilidades Greenbone OpenVAS — el importador original se construyó contra un formato **inventado** que no compartía ni un solo campo con una exportación real (`results[]` vs. el `allHostSubreportEntries[]` real), así que subir un informe real "tenía éxito" en silencio con 0 vulnerabilidades procesadas. **Cambio de modelo de identidad**: una vulnerabilidad Greenbone se identifica ahora por `key = "${oid}@${port}"`, no por `cve` — en la exportación real solo ~4% de los hallazgos llevan CVE (2 de 52 en el fixture real usado para verificación, `docs/mocks/greenbone_SRV-MYGESTR01D.json`); `cve` se conserva como campo de visualización compatible hacia atrás. **Cascada de emparejamiento de CI** de 5 niveles (IP exacta → nombre exacto → hostname exacto → DNS exacto → nombre parcial fuzzy), parada en el primer nivel con resultados, `AMBIGUOUS` ante 2+ candidatos del mismo nivel — nunca adivina, corrigiendo el antipatrón real `ORDER BY LENGTH(name) LIMIT 1` del importador antiguo. **El punto central del release**: staging obligatorio. Subir un informe ya no escribe nunca directamente sobre un CI — crea un `VulnImportBatch` (`PENDING`) con una `VulnImportEntry` por hallazgo (2 tablas Postgres nuevas), que un operador revisa en una UI nueva (`/vulnerabilities/imports` + `/vulnerabilities/imports/[id]`, 4 pestañas independientes Accionables/Informativas/Requieren atención/Reaparecidas) antes de **Aceptar** (transaccional: escritura en los CIs afectados + `VULN_IMPORT_ACCEPT` en `audit_logs`, en una sola `prisma.$transaction`) o **Descartar** (no toca nada). Clasificación NUEVA/EXISTENTE_PENDIENTE/REAPARECIDA con decisión de inclusión por defecto sensata (reaparecida siempre incluida sin importar severidad — es la señal más importante de un reescaneo). Estado nuevo **`REABIERTA`** (lo que se convierte una `REAPARECIDA` al aceptarse, conservando `resolvedAt` y fijando `reopenedAt`) cuenta como abierto en el motor de alertas existente (`alerts/engine.ts`) sin mecanismo de alerta nuevo — deliberado, misma filosofía de reutilización que D4 del conector vCenter. Banda de severidad nueva `INFO` (CVSS 0.0). Endpoint legacy `POST /api/integrations/greenbone` conservado como shim de compatibilidad que delega en la misma lógica de staging; body con forma del mock antiguo ahora devuelve `400` explícito en vez de "éxito" con 0 entradas. `PATCH /api/vulnerabilities` resuelve identidad como `key ?? cve`. Script `backend/scripts/backfill-vuln-keys.js` (idempotente, `--dry-run`) para poblar `key` en entradas anteriores al release. **Bug real cazado en la verificación en vivo, no por revisión de código ni por el test dedicado**: `audit_logs.entity_id` es `varchar(36)` (dimensionado para un UUID desnudo), pero el insert de auditoría de `PATCH /api/vulnerabilities` construía un string compuesto `${ciId}:${key}` que desborda esa columna para cualquier key/cve real — Postgres rechazaba el insert (`22001`) y la petición devolvía `500`. El bug es anterior a este release (existe desde el commit de #172 que envolvió esta escritura en transacción) pero nunca se había disparado porque ningún CI tenía vulnerabilidades reales hasta que este módulo empezó a escribirlas; el test dedicado (`vulnPatchIdentity.test.ts`) reafirmaba la forma antigua defectuosa contra un mock aislado en vez de tocar Postgres real — la brecha ya documentada de este repo ("ningún test toca Postgres real"). Corregido: `entity_id` solo lleva el `ciId` desnudo; la identidad de la vulnerabilidad se movió a la columna `details` (jsonb, sin límite). Verificado en vivo end-to-end contra producción con el fixture real: emparejamiento `EXACT_IP` sin intervención manual, 52 vulnerabilidades sin colisión de `key`, reimportación del mismo fichero clasificando lo ya aceptado como `EXISTENTE_PENDIENTE` sin duplicar, ciclo completo `RESUELTO` → reescaneo → `REAPARECIDA` → aceptar → `REABIERTA` con `reopenedAt` y auditoría `VULN_REOPENED`, host con IP desconocida → `UNMATCHED` → `422 UNRESOLVED_MATCHES` nombrando las entradas bloqueantes. 771/771 tests backend tras el fix, `tsc --noEmit` limpio. **Verificación en navegador real completada a posteriori** (tras resolver un problema de entorno: `npx playwright install chromium` descarga un binario autocontenido sin sudo, a diferencia del canal `chrome` del sistema que sí lo exige; un proceso MCP huérfano quedó bloqueando el perfil del navegador entre reconexiones y hubo que matarlo manualmente — configuración final en `.mcp.json`, reutilizable en próximas sesiones): confirmadas visualmente en Chrome real las insignias CISA KEV/explotación activa en la fila sin expandir, `exprtRating` separado del pill de severidad, el checkbox reformulado, el menú "Security" visible solo para ADMIN/SOC, y las etiquetas de clasificación reformuladas — todo coincidiendo exactamente con lo ya verificado por API. **Limpieza de artefacto histórico**: `docs/mocks/greenbone_sample.json` (el mock antiguo, confirmado sin ningún campo en común con el formato real) renombrado a `docs/mocks/greenbone_sample_LEGACY_INVENTADO.json` para que nadie vuelva a diseñar contra él por error. **Carencia conocida, no implementada en este release**: los batches `PENDING` abandonados no se purgan automáticamente — la especificación proponía plegarlo en el cron de mantenimiento existente, pero quedó fuera de alcance. Documentación: `docs/INTEGRATIONS.md` § 9, `docs/USER_MANUAL(.en).md` § 10, `docs/SYSADMIN_MANUAL(.en).md`, `docs/ARCHITECTURE(.en).md`.

**Continuación (2026-07-30, misma rama, liberada junto con el bloque anterior en el mismo tag `v3.6.0` — informalmente "v3.6.1"):** segunda fuente de vulnerabilidades sobre el mismo staging — **conector CrowdStrike Spotlight** — más un fix de UX de la pantalla de revisión reportado por el usuario. El conector CrowdStrike ya existente (`POST /api/integrations/crowdstrike`) ingiere estado de agente/EDR de Falcon (`{devices:[...]}`), funcionalidad real sin tocar; Spotlight es un producto CrowdStrike distinto (gestión de vulnerabilidades), verificado contra una exportación real proporcionada por el usuario (`docs/mocks/crowdstrike_SRV-MYGESTR01D.json`, 841 registros para el CI `SRV-MYGESTR01D`) — un array JSON plano en el nivel superior, forma y dominio distintos tanto del mock de agente/EDR como de Greenbone. **Modelo de identidad**: `vulnerability_id` solo, nunca `vulnerability_id + product` — CrowdStrike emite un registro por producto afectado de la misma vulnerabilidad, así que los 841 registros crudos colapsan a **635** IDs distintos; los registros de un mismo grupo se fusionan (`products` unión, `solution` deduplicada, "Reopened gana" si cualquier registro del grupo lo tiene). `vulnerability_id` no siempre es CVE — un registro real (`CS-V26-A757135`) no tiene `cve_id`, es un ID propio de CrowdStrike. **Severidad CVSS vs. `exprtRating`**: la severidad se deriva de `base_score` reutilizando las mismas bandas `scoreToSeverity` ya establecidas para Greenbone; `exprtRating` (valoración propia de CrowdStrike con IA) se almacena y muestra como señal **separada**, deliberadamente nunca fusionada — discrepan en ~65% de los registros reales. **Premarcado forzado CISA KEV/explotación activa**: `cisaKev`+`cisaDueDate` y `exploitStatus` fuerzan inclusión premarcada con independencia de la banda de severidad; la allowlist de etiquetas que cuentan como "explotación activa" (`ACTIVE_EXPLOITATION_LABELS` en `classifier.ts`) es explícita — `"Actively used (critical)"`/`"Easily Accessible (high)"` cuentan, `"Unproven"`/`"Available (medium)"` no. **Segunda ruta a `REAPARECIDA`**: además de la ruta original (copia almacenada del CMDB en `RESUELTO`), ahora también se clasifica `REAPARECIDA` si el `externalStatus` entrante es `"Reopened"` — la propia señal de reapertura de CrowdStrike, comprobada antes que las demás ramas porque puede prevalecer sobre ellas, incluso sin ningún registro previo en el CMDB (los formatos de identidad Greenbone/CrowdStrike son disjuntos y no se cruzan hoy); al aceptar, si no hay match real que reabrir, cae a almacenarse como entrada nueva en vez de fallar. **Fix real de esta ronda**: `cisaKev` (el único de los 8 campos nuevos no-nullable) se refresca con semántica sticky-true (`entry.cisaKev || existing.cisaKev`) en la ruta de reopen, para que una marca CISA KEV ya fijada no desaparezca silenciosamente en un refresco posterior que no la vuelva a traer. **`POST /api/integrations/crowdstrike` pasa a autodetectar formato**: `{devices:[...]}` → flujo agente/EDR existente sin tocar; array plano → mismo staging que Greenbone; cualquier otra cosa → `400` nombrando ambos formatos aceptados. El override de 20MB de body-size ya aplicado a `/api/vuln-import/upload` en v3.6.0 se aplica ahora también a `/api/integrations/crowdstrike` (una exportación Spotlight real de un solo host ronda 686KB), registrado en `index.ts` antes del parser global — misma clase de bug histórico evitada deliberadamente. **Esquema**: `VulnImportEntry` gana 8 columnas nullable (`products`, `exprt_rating`, `cisa_kev` not-null-default-false, `cisa_due_date`, `exploit_status`, `days_open`, `external_status`, `cvss_version`); `oid`/`port` (Greenbone) pasan a nullable ya que CrowdStrike no tiene ninguno de los dos; el JSON `Vulnerability` persistido gana los mismos campos opcionales más `source` que ahora varía de verdad. **Fix de UX de la pantalla de revisión, independiente de CrowdStrike**: el control de inclusión/exclusión era un botón de verbo imperativo ("Excluir"/"Incluir") que mostraba el estado actual pero se leía como una orden — confuso en una entrada ya excluida. **El comportamiento del clasificador se verificó de forma independiente como correcto** contra datos reales (ya pendientes sí vienen excluidas por defecto) — solo cambió el texto, ninguna lógica de backend. Ahora es una casilla con etiquetas orientadas al resultado ("Se importará"/"No se importará"); la etiqueta de visualización de `EXISTENTE_PENDIENTE` (no su valor/clave) se reformuló a "Ya existe en este CI" con explicación de que se refresca, no se reimporta. **Verificado en vivo contra producción** con el fixture real: 841→635 fusión correcta; CI emparejado por IP sin intervención manual; una CVE con 3 productos fusionada en una entrada; el ID propio de CrowdStrike sin CVE importado con lista vacía; 3 vulnerabilidades CISA KEV reales premarcadas; 28 `Reopened` distintos clasificados `REAPARECIDA` incluso sin registro previo (incluido el caso "sin match + Reopened"); re-subir el mismo fichero no produjo duplicados; el formato legacy agente/EDR sigue funcionando sin cambios por el mismo endpoint; formato no reconocido → `400` nombrando ambas formas. 847/847 tests backend, `tsc --noEmit` limpio. **Verificación en navegador real completada a posteriori**, misma sesión de resolución de entorno que v3.6.0 (ver arriba): insignia roja "CISA KEV — due 10/16/2023" (fecha real del fixture) y insignia naranja "Active exploitation" confirmadas visibles en la fila sin expandir, y confirmadas como señales independientes (una entrada con explotación activa pero sin KEV las muestra por separado); `EXPRT RATING: Critical` confirmado separado del pill de severidad "High" de la fila; panel de detalle con `DAYS OPEN`/`CVSS VERSION`/`AFFECTED PRODUCTS` (JDK 1.8.0 + JRE 1.8.0 fusionados) confirmado; menú "Security" confirmado visible solo para ADMIN, oculto para AUDITOR. `docs/mocks/crowdstrike_sample.json` (mock de agente/EDR, distinto del ya renombrado `greenbone_sample_LEGACY_INVENTADO.json` — este NO se renombra, sigue siendo un flujo real vivo, solo nunca verificado contra una exportación Falcon real) gana una clave `_note` aclarando la distinción frente a `crowdstrike_SRV-MYGESTR01D.json`. Documentación actualizada: `docs/INTEGRATIONS.md` § 9.12, `docs/USER_MANUAL(.en).md` § 10, `docs/SYSADMIN_MANUAL(.en).md`, `docs/ARCHITECTURE(.en).md`.
- **v3.5.12** ✅ LIBERADA (2026-07-28): spec+plan escritos por Opus, ejecutados por Sonnet vía subagent-driven-development (6 agentes en paralelo — B1-B4 backend secuencial + F1-F5 frontend paralelas, cada uno en su propio worktree/branch, con commits incrementales), más una vuelta adicional de trabajo directo (F6 i18n, F-INT integración, feature FESTIVO añadida a mitad de sesión, fix de un bug real cazado en la verificación en vivo). **UX de la vista**: tablas de departamento a `table-fixed`+`colgroup` con anchos porcentuales declarados (antes `w-full`+`min-w-*`, dependiente del contenido) — todas las tablas quedan con el mismo reparto de columnas por construcción; altura de fila fija `h-11` (antes `min-h-[3.5rem]` variable) con el nombre del estado en `line-clamp-2`. **Resumen semanal restringido**: `ScheduleRow.summary` pasa a opcional y se **omite en servidor** (no se envía a 0) para quien no sea ADMIN o gestor de *ese* departamento concreto — `canViewSummary()` delega en `canUserEditDepartment` (una sola fuente de verdad con `canEdit`, no una copia que pudiera divergir); decisión explícita de no usar "MANAGER o superior" por rango de rol. **Alertas ocultas al publicar**: `AlertPanel` solo en `DRAFT`; el calendario pasa a ancho completo al ocultarse. **Filtro por trabajador** (2 endpoints nuevos: `GET /users?q=` sin `email` en la respuesta por minimización GDPR Art. 5.1.c, y `GET /user/:userId/entries?from=&to=` acotado a 62 días, visibilidad en el `WHERE` como el resto del módulo, `weeklyTargetHours` agrupado por semana para no divergir de la vista de departamento). **Vista mensual de departamento** (`GET /` gana `from`/`to` sobre `weekStart` sin sustituir el exacto preexistente, acotado a 6 semanas; semana sin horario se muestra explícita, nunca se omite). **Impresión** — CSS `@media print` + `window.print()` sin dependencias nuevas (descartado jsPDF/html2canvas por superficie CVE y saltos de página manuales, A06), `break-inside: avoid` para no partir tablas salvo que excedan una página entera; `POST /audit/print` registra la exportación (ISO 27001 A.8.15) con `user_email` siempre de la sesión (verificado en vivo: un valor falsificado en el body se ignora), sin bloquear la impresión si el registro falla (D7 — el dato ya está en pantalla). **Estados nuevos `FESTIVO`/`FESTIVO_LOCAL`** (añadidos a mitad de sesión, a petición del usuario): TEXT+Zod, sin migración de BD; además de contar 0h como `VACACIONES`, ahora **reducen el objetivo de horas semanales** de esa semana concreta en vez de contarse como déficit contra un objetivo plano de 40h (`computeEffectiveWeeklyTarget`, deliberadamente más estrecho que `NON_WORKING_STATUSES` — baja médica/paternidad/ausencia/viaje siguen contando como déficit). Verificado en vivo contra producción con datos reales: una semana real de 5 días `VACACIONES` reporta `weeklyTargetHours: 2`, exactamente `40 - (4×8 + 6)`. **Bug real cazado en la verificación en vivo**: el botón de imprimir de la vista departamento-semana enviaba el id del horario como `targetId` en vez del id del departamento (que es lo que `POST /audit/print` siempre espera para ambos scopes `DEPARTMENT_*`) — toda impresión desde esa vista devolvía 404 en silencio en vez de auditarse; confirmado con la cuenta AUDITOR de pruebas, corregido, redesplegado y reconfirmado. **Incidente operativo durante el despliegue**: ejecutar `podman-compose` desde el worktree de integración (no desde el checkout principal) hizo que `DOCUMENTS_STORAGE_PATH=./document-storage` (ruta relativa) resolviera contra el directorio equivocado — el backend entró en crash-loop por un `EACCES` al `mkdir`, **antes** de tocar ningún dato real (los certificados TLS usan el volumen nombrado `cmdb-tls-certs`, no rutas relativas, así que no se vieron afectados); corregido desplegando siempre desde `/opt/cmdb-enterprise-platform`. 607/607 tests backend (11 nuevos: computeEffectiveWeeklyTarget + regresión FESTIVO), `tsc --noEmit` limpio backend+frontend. **Segunda fase — 6 rondas de corrección guiadas por el usuario probando en navegador real** (Chrome no era instalable en este entorno, sin sudo, así que la verificación automatizada solo llegó a la capa de API; todo lo siguiente lo encontró el usuario, no la revisión de código ni los tests): (1) **crash de la vista de trabajador** — `WorkerEntryItem` del frontend declaraba forma anidada (`{entry: {...}}`) contra la forma plana que el backend siempre envió; construidos por agentes paralelos distintos (B3/F4) contra un contrato en prosa, y `tsc` no puede cazarlo porque la respuesta del `fetch` se castea sin validación en ejecución — **lección: contratos compartidos entre tareas paralelas deben derivarse del tipo del servidor o validarse en runtime**; (2) impresión capturaba el cromo de la app (barra lateral, fondos grises, barras de scroll) porque `AppShell.tsx`/`TopBar.tsx` viven fuera del módulo y su CSS no los alcanzaba; (3) **`@page` con nombre para orientación por vista NO funciona** — Chromium no re-maqueta al cambiar el tamaño de página a mitad de documento: diagnosticado sobre el PDF real que subió el usuario al repositorio documental y analizado con `pypdf` (p1 apaisada solo con la cabecera, p2-3 verticales con las tablas maquetadas al ancho apaisado, ~398pt fuera del borde); sustituido por **una sola `@page` reescrita desde JS por trabajo de impresión** (`usePrintPageOrientation`); (4) `table-layout: auto` en impresión fue el error que rompió el encaje (con `auto` manda el contenido, no la fuente — por eso encoger la letra no arreglaba nada); revertido a `fixed`; (5) filas fijas del calendario pintando sobre el título por conflicto de `z-index` (cabecera `z-10` vs `thead` `z-20`/primera columna `z-30`, sin contexto de apilamiento propio) → cabecera a `z-40`; (6) bloques de color de tamaños distintos por altura fija `h-11` en el botón → `h-full` (la celda se estira a la fila y el porcentaje se resuelve contra esa altura usada), conservando el `p-0.5` del `<td>`. Detalle documentado en `docs/STAFF_SCHEDULE.md` §17.6.1-17.6.3 y §17.9. Spec: `docs/internal/specs/2026-07-28-v3.5.12-staff-schedule-ux-print.md`. Plan: `docs/internal/plans/2026-07-28-v3.5.12-staff-schedule-ux-print.md`.
- **v3.5.11** ✅ LIBERADA (2026-07-27): tres correcciones/carencias del módulo de Horarios de Personal reportadas en uso real. **Estado nuevo `INTENSIVO_TELETRABAJO`** (jornada intensiva desde casa): no deduce descanso (como `INTENSIVO`), consume cuota de teletrabajo (como `TELETRABAJO`) y queda fuera de `FLEX_RANGE` — las tres semánticas se centralizan en dos allowlists nuevas de `schemas.ts` (`INTENSIVE_STATUSES`, `TELEWORK_STATUSES`) en lugar de comparar contra literales dispersos, de modo que un cuarto estado futuro no obligue a cazar comparaciones sueltas. **Cuota de teletrabajo por trabajador**: `users.telework_full` / `telework_quota_days` / `telework_quota_pct`, todos opcionales (ninguno fijado = comportamiento anterior exacto), editables por ADMIN vía `PUT /api/staff-schedule/users/:userId/telework-quota` (transaccional + `AuditLog`) y desde el panel de configuración. Prioridad **total > días > porcentaje > tope del departamento** (D1); el porcentaje se calcula sobre los días L-V del **mes natural**, no sobre los días ya planificados (D2 — si no, el tope se movería según se rellena el horario). Motivación real: un trabajador con 100% teletrabajo por motivos médicos disparaba la alerta de cuota de forma permanente; ahora queda exento (`cap === null`, no `cap = Infinity`, para que la exención sea explícita en el tipo). Rangos validados en Zod **y** con `CHECK` en la BD, para que el invariante aguante una escritura que no pase por la API. **Fix `PRESENCE_PCT` siempre 0.0%**: la regla exigía que la jornada de un trabajador *contuviera entera* la franja núcleo (`start <= presenceStart && end >= presenceEnd`); con la configuración real (franja 09:00–18:00, 9 h) y jornadas de ~8.5 h la condición era **insatisfacible**, así que el contador daba 0 hubiera quien hubiera en la oficina. Ahora cuenta el **solape** con la franja, y el denominador solo incluye a quien está disponible ese día (vacaciones, bajas, ausencias y viajes fuera — antes una semana de vacaciones incumplía el mínimo por definición); una entrada `PRESENCIAL` sin horas cuenta como presente, porque es la forma en que `createSchedule` siembra una semana nueva y si no toda semana recién creada reportaría 0%. Verificado en vivo contra el departamento real: día 2026-07-27 pasa de `0.0%` a `33.3%` (1 presente de 3 disponibles, 1 de vacaciones + 1 de baja fuera del denominador), aritmética contrastada contra las filas de la BD; el estado nuevo guarda 6 h netas y suma a la cuota; los cuatro caminos de resolución de cuota (días / total / porcentaje / rango inválido) ejercitados por API. 559/559 tests (14 nuevos), `tsc --noEmit` backend limpio, build de frontend typecheckeado. **Hallazgo operativo**: construir con `podman build -t` usando un tag distinto del `image:` del compose (`cmdb-prod_backend` en vez de `cmdb-backend`) deja el stack corriendo la imagen anterior **sin ningún síntoma** — health 200, contenedores "Up", logs limpios; verificar el `dist` dentro de la imagen recién construida NO prueba qué imagen corre el contenedor. Lo cazó la verificación por API (un campo nuevo ausente en la respuesta), no el chequeo del build; comprobar siempre `podman inspect <ctr> --format '{{.Image}}'` tras el `up`.
- **v3.5.10** ✅ LIBERADA (2026-07-26): control de acceso por grupo de AD + rol MANAGER + displayName + sincronización de usuarios. **Login**: con `LDAP_REQUIRED_GROUP` configurado (CN o DN), solo los miembros del grupo pueden entrar vía LDAP; pertenencia anidada por `LDAP_MATCHING_RULE_IN_CHAIN` (configurable a directa). Nuevo `services/ldapDirectory.ts` separado de `services/ldap.ts` — aquél prueba credenciales, éste consulta el directorio con la cuenta de servicio; la frontera es deliberada. La puerta se evalúa con el `sAMAccountName` autoritativo ya resuelto y **antes** de cualquier auto-heal/auto-provisión: quien no tiene acceso no llega a tener fila. Tres ramas: pertenece → flujo normal; no pertenece → `401` con **el mismo mensaje que credenciales erróneas** (no permitir enumeración) + `active=false` + `LDAP_GROUP_DENIED` en una transacción; **no verificable → `401` fail-closed** (D7 — degradar a "permitir" convertiría una caída del directorio en una desactivación silenciosa de la política). Cuentas locales al margen de las tres ramas: un DC caído nunca deja fuera al administrador (ISO 22301). **Rol**: `WORKER`→`MANAGER` vía `ALTER TYPE … RENAME VALUE` (filas conservan su rol; había 0 WORKER en prod). MANAGER = AUDITOR fuera de horarios (gana DCIM y rango 2 de informes) **salvo** `audit-trail`, vetado por denylist explícita porque el rango lineal se lo daría; `requireAudit` sigue excluyéndolo. **Visibilidad de horarios**: VIEWER recupera el módulo pero solo ve publicados; AUDITOR **deja de ver borradores** (antes veía todo); MANAGER ve además cualquier estado de los departamentos que gestiona; ADMIN todo. Filtro en la cláusula `WHERE` de Prisma (`buildScheduleVisibilityFilter`), nunca post-filtrado (A01), y fuera de alcance → **404, no 403**. Rol desconocido → rama más restrictiva (fail-closed). El masking GDPR Art.9 intacto: nunca dependió de que VIEWER estuviera bloqueado. **displayName**: `users.display_name` poblado en auto-provisión y refrescado en cada login; `displayLabel()` con respaldo a username en calendario/alertas/selectores/panel de usuarios. La erasure GDPR no necesitó cambios — hace `DELETE` de la fila, no anonimización por campos (verificado en el handler, no asumido). **Sincronización**: `runLdapGroupSync()` única, compartida por el botón ADMIN y n8n; diff aislado en función pura y testeada; nunca borra (desactiva) ni reescribe roles (D9: AD posee identidad, operador posee gobernanza); usuarios manuales intocables por cláusula de BD; auditoría transaccional; lock en proceso. Se **retiran** `/api/internal/users/ldap-sync*` y el workflow que calculaba el diff con nodos LDAP+Code — segunda implementación de la misma regla, con capacidad de divergir. Nuevo workflow `LDAP Group Sync` (Schedule 03:00 → endpoint interno), activo solo si `USE_LDAP=true` **y** hay grupo. 6 vars nuevas, todas opcionales (vacío = comportamiento anterior). `ldapts` 8.2.0 directo (la 7.x arrastra el CVE de uuid GHSA-w5hq-g745-h8pq; 8.1+ limpia). **Hallazgo**: el typecheck del frontend cazó un `WORKER` en `AuthContext` que el grep no encontró — merece la pena instalar las deps del frontend en el worktree en vez de fiarlo al build del contenedor. Plan: `docs/internal/plans/2026-07-26-v3.5.10-ldap-group-manager-role.md`, spec: `docs/internal/specs/2026-07-26-ldap-group-gate-manager-role-design.md`. **Segunda tanda (2026-07-27, misma versión, plan `docs/internal/plans/2026-07-27-v3.5.10-staff-schedule-refinements.md`)**: 8 defectos/carencias reales detectados en la verificación en vivo con datos de producción (departamento System & Infrastructure, 5 miembros, 1 responsable). Confirmados y corregidos: `FLEX_RANGE` no debe aplicar a `INTENSIVO` (validationEngine.ts); panel de configuración sin GET de managers/miembros (2 endpoints nuevos, lectura pura); botón de horas semanales dependía de un selector de otra sección y nunca se habilitaba (selector propio + prefill); orden de filas por `userId` en vez de responsable-primero (`sortRowManagerFirst`, comparador puro testeado); **causa raíz confirmada en BD** de "semanas clonadas sin trabajadores": horarios `StaffSchedule` con 0/20/25/30 `ScheduleEntry` según la membresía del departamento en el momento de crear/clonar — sin mecanismo para reconciliar. Resuelto con `DELETE /:id` (solo DRAFT, D10) + `POST /:id/sync-members` (no destructivo, añade solo a quien falte) — cubren tanto el horario vacío como el trabajador nuevo que se incorpora a planificación ya hecha. Descartados como no-issue tras revisar código/datos: el desplegable de roles ya tenía MANAGER (etiqueta "Gestor"); ADMIN ya podía editar cualquier departamento (`canUserEditDepartment`); el nombre no aparecía en la columna Persona por caché del navegador, no por bug — confirmado con la API devolviendo `displayName` correcto para 4/5 trabajadores antes de que el usuario recargara.
...
- **v3.5.9** ✅ LIBERADA (tag `v3.5.9`, PR #195 feature→develop + PR #196 develop→main, release en GitHub, main/develop sincronizados, 2026-07-25): rework completo de Horarios de Personal, ejecutado vía subagent-driven-development (18 tareas, revisión individual por tarea + revisión final de rama completa en opus). **Fix cálculo de horas** (issue reportado por el usuario): `computeNetHours` saltaba el descanso todos los viernes independientemente del estado, no solo en `INTENSIVO` — inflaba una semana normal de 5×8h a 40.5h. Corregido: el descanso se aplica siempre salvo `INTENSIVO`. **GUARDIA deja de ser un estado exclusivo**: ahora es un complemento booleano `onGuard` que puede coexistir con cualquier estado (p.ej. TELETRABAJO + de guardia a la vez); unicidad de "un solo trabajador de guardia por departamento y día" forzada a nivel BD con índice único parcial (`schedule_entries_on_guard_unique` sobre `(department_id, date) WHERE on_guard = true`, `ScheduleEntry.departmentId` denormalizado desde `StaffSchedule` para poder indexarlo) + regla de validación `GUARDIA_UNIQUE` como aviso temprano antes de publicar; migración de datos de filas `GUARDIA` preexistentes a `onGuard=true`+`status=PRESENCIAL`. **Clonado rediseñado**: el endpoint `POST /:id/clone` ahora acepta `targetWeekStart` elegido por el usuario (antes fijo a "semana siguiente", fallaba con error confuso si esa semana concreta ya existía aunque hubiera otras libres) + botón nuevo "Importar semana anterior". **Horas semanales por trabajador**: override opcional (`User.weeklyTargetHours`) editable por ADMIN, alimenta tanto la validación V-WEEKLY_HOURS como el autorelleno de hora de salida en el editor de entrada. **Contador de teletrabajo separado** en semana/mes (antes solo mostraba el acumulado del mes mal etiquetado como semanal). **Rol nuevo `WORKER`**: equivalente a VIEWER en toda la aplicación salvo acceso de lectura a Horarios de Personal (todos los departamentos, mismo enmascarado GDPR que AUDITOR). **Vista "Todos los departamentos"** de solo lectura (antes no mostraba nada). Botón "Aplicar a toda la semana". **Dos bugs reales encontrados y corregidos mediante verificación end-to-end en vivo contra producción real, no cazados por 2 rondas de revisión de código previas**: (1) el manejador de la violación P2002 del índice único de guardia comprobaba si el nombre del índice aparecía como substring de `err.meta.target`, pero Prisma reporta ahí la tupla de columnas (`['department_id','date']`), no el nombre del índice — un conflicto real de guardia devolvía 500 en vez de 409; corregido comparando la tupla de columnas directamente. (2) la revisión final de toda la rama detectó que al añadir el rol `WORKER`, la puerta de acceso de DCIM (`dcim/middleware.ts`) solo bloqueaba `VIEWER`, dejando pasar a `WORKER` con acceso de lectura a DCIM no previsto — corregido y verificado en vivo (`WORKER` → 403 en `/api/dcim/*`, sigue 200 en `/api/staff-schedule/*`). También corregido durante desarrollo (antes de la review final): fuga GDPR Art. 9 en los contadores agregados `guardDays` (no excluían entradas con estado de baja médica, dado que `onGuard` pasó a ser ortogonal a `status`). 479/479 tests, `tsc --noEmit` limpio backend+frontend, stack de producción completo reconstruido y redesplegado, 8/8 comprobaciones manuales en vivo contra el departamento real "System & Infrastructure" incluyendo el escenario exacto reportado (`weeklyNetHours: 40`, no 40.5). Quedan 3 horarios DRAFT de prueba en ese departamento real (semanas futuras 2026-09-07/-14/-28) — no existe endpoint de borrado de horarios en el módulo, inofensivos pero visibles. Plan: `docs/superpowers/plans/2026-07-24-staff-schedule-v3.5.9.md`.
- **v3.5.8** ✅ LIBERADA (tag `v3.5.8`, PR #192 fix→develop + PR #193 develop→main, release en GitHub, main/develop sincronizados, 2026-07-24): fix issue #191 — `PATCH /api/users/:id/role` devolvía 500 en todo cambio de rol (bug detectado al promocionar un usuario recién creado vía SSO/AD, pero no específico de AD). Causa raíz: `UPDATE "users" SET role = ${role}` vinculaba `role` como parámetro Prisma `text` contra la columna enum Postgres `users.role` (`"UserRole"`), sin cast implícito (`42804`/`P2010`). Fix de una línea: cast explícito `::"UserRole"`, coherente con el `::uuid` ya presente. Confirmado como único sitio con este patrón en todo el repo. No relacionado con #172 (v3.5.6) — el bug preexiste al commit original del panel de usuarios. 477/477 tests, `tsc --noEmit` limpio, verificación contra Postgres real (bind aislado + `dist` compilado). **Follow-up propuesto, no ejecutado en este release**: extraer la administración de usuarios (`users`, no auth/SSO/MFA) a `modules/users/` como v3.6.0 — habilitaría tests supertest reales que hubieran cazado este bug (la suite actual mockea `$executeRaw` en todos los módulos, ningún test toca Postgres real).
