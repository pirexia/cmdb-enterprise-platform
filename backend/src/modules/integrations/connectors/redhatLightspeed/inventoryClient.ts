...
async function getOne<T>(url: string, token: string, context: string): Promise<T> {
  ...
  if (!Array.isArray(body?.results) || body.results.length === 0) {
    throw new Error(`Red Hat Insights Inventory API returned no results for ${context}`);
  }
...
export async function getHostIdentity(baseUrl: string, token: string, inventoryId: string): Promise<HostIdentity> {
  const encodedId = encodeURIComponent(inventoryId);
  const host = await getOne<InventoryHostResult>(
    `${baseUrl}/api/inventory/v1/hosts/${encodedId}`, token, `hosts/${inventoryId}`,
  );
  const profile = await getOne<SystemProfileResult>(
    `${baseUrl}/api/inventory/v1/hosts/${encodedId}/system_profile?fields[system_profile]=operating_system`,
