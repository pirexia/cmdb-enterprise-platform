...
// Emit a vuln-import module audit log record (insert-only — ISO 27001 A.8.15).
//
...
export async function vulnImportAudit(
  ...
  await db.$executeRaw`
    ...
    VALUES (gen_random_uuid(), ${action}, ${entity}, ${entityId}::uuid, ${userEmail}, ${details ? JSON.stringify(details) : null}::jsonb, now())
  `;
}
