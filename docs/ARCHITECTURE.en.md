...
**Third source: Red Hat Lightspeed (v3.7.0) — live-pull, not a file upload.** Unlike Greenbone/CrowdStrike, this connector doesn't accept pasted JSON: `backend/src/modules/integrations/connectors/redhatLightspeed/` implements its own connector (`config.ts` + `tokenClient.ts` OAuth2 client_credentials against `sso.redhat.com` + `vulnClient.ts`/`inventoryClient.ts` against the Insights Vulnerability/Inventory APIs + `lifecycleClient.ts` against Red Hat's public Product Life Cycle API + a pure `mapper.ts`) whose orchestration (`service.ts`, `runRedHatLightspeedImport()`) walks every visible RHEL system, maps its CVEs into the same `ParsedVulnEntry` shape Greenbone/CrowdStrike produce, and creates a staging batch exactly like `uploadReport()` does. Identity is the CVE itself (`synopsis`), not a scanner-specific id — Red Hat's own data model is already CVE-centric. Accepting a batch from this source gives `acceptBatch()` two new behaviors, both scoped to `batch.source === 'redhat-lightspeed'`: `CI.operatingSystemId` correction (always refreshed, unlike `hypervisorId`) with EOL/EOS dates backfilled from the public lifecycle API the first time each OS version is created, and a closure sweep that marks `RESUELTO` any `source='redhat-lightspeed'` vulnerability no longer present in the batch — possible only because this source gives a complete picture of the CI, unlike a manual report. See `docs/INTEGRATIONS.md` § 9.13 for the full detail.
