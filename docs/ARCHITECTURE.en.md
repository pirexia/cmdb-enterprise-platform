...
### Greenbone vulnerability import (v3.6.0, on `develop`)

> Branch `feature/v3.6.0-greenbone-real-format`, not yet tagged or merged to `main` as of writing.

Self-contained module `backend/src/modules/vuln-import/`, mounted at `/api/vuln-import` — replaces the original Greenbone importer, which was built against an invented format sharing zero fields with a real OpenVAS export. It follows the same **pure-function / orchestration-layer** split already used by the vCenter connector (`connectors/vcenter/` → pure mapper + HTTP client, vs. `vcenterService.ts` → orchestration):

- **`parser.ts`** — pure, no I/O. Validates the real Greenbone report shape (`GreenboneReportSchema`, Zod) and normalises each finding into a `ParsedVulnEntry`; explicitly rejects the old format (`results[]`) with a typed error instead of silently "succeeding" with 0 entries.
- **`matcher.ts`** — pure except for the single `$queryRaw` query it runs. A 5-level cascade (exact IP → exact name → exact hostname → exact DNS → partial name) matches a finding's host against an existing CI; it never auto-picks between 2+ candidates at the same level (`AMBIGUOUS`).
- **`classifier.ts`** — pure, no I/O, no Prisma. Compares each incoming finding against what's already stored on the matched CI and decides `NUEVA` / `EXISTENTE_PENDIENTE` / `REAPARECIDA` along with its default inclusion decision.
- **`service.ts` / `queries.ts` / `audit.ts` / `router.ts`** — orchestration and persistence layer: create/read `VulnImportBatch`/`VulnImportEntry`, apply an accepted batch to the affected CIs inside a single `prisma.$transaction` together with the `VULN_IMPORT_ACCEPT` record in `audit_logs`, and expose the 7 REST endpoints under `/api/vuln-import`.

A Greenbone vulnerability's identity is `key = "${oid}@${port}"`, not `cve` — in a real export, only ~4% of findings carry an associated CVE. No upload ever writes to a CI directly: it creates a `PENDING` batch that an operator reviews and decides on, finding by finding, before "Accept" (transactional) or "Discard" (touches nothing).

**Second source, same branch, not yet tagged: CrowdStrike Spotlight.** The module went from single-source (Greenbone) to genuinely multi-source: `parser.ts` (Greenbone) and **`crowdstrikeParser.ts`** (CrowdStrike Spotlight — vulnerability management, a different product from the CrowdStrike Falcon agent/EDR integration `/api/integrations/crowdstrike` already had) both produce the **same** `ParsedVulnEntry` shape, consumed without branching by `matcher.ts`/`classifier.ts`/`service.ts`/`router.ts` — none of the four has a source-conditional branch. A CrowdStrike vulnerability's identity is `vulnerability_id` (not `vulnerability_id + product`: CrowdStrike emits one record per affected product, and `crowdstrikeParser.ts` merges them by `vulnerability_id` before producing each `ParsedVulnEntry`). `classifier.ts` gains a second path to `REAPARECIDA` — CrowdStrike's own reopen signal (`externalStatus === 'Reopened'`), independent of the original path based on the CMDB's stored copy being `RESUELTO` — plus forced premarking on CISA KEV/active exploitation that ignores the severity band. `POST /api/integrations/crowdstrike` auto-detects by structure (`{devices:[...]}` → the existing, untouched agent/EDR flow; flat array → the same staging Greenbone uses) and shares the same 20MB body-size override as `/api/vuln-import/upload`. See `docs/INTEGRATIONS.md` § 9.12 for the full detail (merge model, CVSS vs. `exprtRating`, `cisaKev` sticky-true).

Two new frontend routes: `frontend/app/vulnerabilities/imports/` (batch list, with the upload button) and `frontend/app/vulnerabilities/imports/[id]/` (the review screen, with the four Actionable/Informational/Needs attention/Reappeared tabs). The legacy `POST /api/integrations/greenbone` endpoint is kept as a compatibility shim that delegates to the same staging logic. See `docs/INTEGRATIONS.md` § 9 for the full architecture, the matching cascade, and the real audit-log bug (`entity_id` `varchar(36)` overflow) found and fixed during this release's live verification.

**Third source: Red Hat Lightspeed (v3.7.0) — asynchronous live-pull, not a file upload.** Unlike Greenbone/CrowdStrike, this connector doesn't accept pasted JSON: `backend/src/modules/integrations/connectors/redhatLightspeed/` implements its own connector (`config.ts` + `tokenClient.ts` OAuth2 client_credentials against `sso.redhat.com` + `vulnClient.ts`/`inventoryClient.ts` against the Insights Vulnerability/Inventory APIs + `lifecycleClient.ts` against Red Hat's public Product Life Cycle API + a pure `mapper.ts`) whose orchestration (`service.ts`, `runRedHatLightspeedImport()`) walks every visible RHEL system, maps its CVEs into the same `ParsedVulnEntry` shape Greenbone/CrowdStrike produce, and creates a staging batch exactly like `uploadReport()` does. Identity is the CVE itself (`synopsis`), not a scanner-specific id — Red Hat's own data model is already CVE-centric.

`POST /api/integrations/redhat-lightspeed/import` does not run this walk synchronously: it responds `202 {batchId}` immediately and `runRedHatLightspeedImport()` continues in the background inside the same Node process — no new queue or worker. The batch row itself (`vuln_import_batches`, new columns `progress_phase`/`progress_current`/`progress_total`/`error_message`, new states `RUNNING`/`FAILED`) is the progress-tracking mechanism; a backend restart with a batch stuck in `RUNNING` marks it `FAILED` automatically on boot (restart orphan). Batch creation was split into `createBatchShell` (short transaction) → `writeBatchEntries` (500-row chunks, no surrounding transaction) → `finalizeBatch` (short transaction) — the same refactor also covers the manual Greenbone/CrowdStrike uploads, which before this were equally at risk of a Prisma `Transaction already closed` with sufficiently large files.

Accepting a batch from this source gives `acceptBatch()` one new behavior scoped to `batch.source === 'redhat-lightspeed'`: `CI.operatingSystemId` correction (always refreshed, unlike `hypervisorId`) with EOL/EOS dates backfilled from the public lifecycle API the first time each OS version is created. The automatic closure of absent vulnerabilities (formerly a sweep exclusive to this source, computed on accept) was generalized to all 3 sources and moved to upload/import time (`computeAbsentClosures()` in `classifier.ts`, `RESUELTA_AUSENTE` classification, reviewable by the operator before accepting, re-verified inside `acceptBatch()`'s transaction) — see `docs/INTEGRATIONS.md` §§ 9.13–9.17 for the full detail.
