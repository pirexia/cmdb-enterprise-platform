...
**Tercera fuente: Red Hat Lightspeed (v3.7.0) — live-pull, no subida de fichero.** A diferencia de Greenbone/CrowdStrike, este conector no acepta un JSON pegado: `backend/src/modules/integrations/connectors/redhatLightspeed/` implementa un conector propio (`config.ts` + `tokenClient.ts` OAuth2 client_credentials contra `sso.redhat.com` + `vulnClient.ts`/`inventoryClient.ts` contra las APIs de Insights Vulnerability/Inventory + `lifecycleClient.ts` contra la API pública de ciclo de vida de productos de Red Hat + `mapper.ts` puro) cuya orquestación (`service.ts`, `runRedHatLightspeedImport()`) recorre cada sistema RHEL visible, mapea sus CVEs a la misma forma `ParsedVulnEntry` que Greenbone/CrowdStrike, y crea un lote de staging exactamente igual que `uploadReport()`. La identidad es la propia CVE (`synopsis`), no un identificador de escáner — el modelo de datos de Red Hat ya es CVE-céntrico. Al aceptar un lote de esta fuente, `acceptBatch()` gana dos comportamientos nuevos, ambos acotados a `batch.source === 'redhat-lightspeed'`: corrección de `CI.operatingSystemId` (siempre refrescado, a diferencia de `hypervisorId`) con relleno de fechas EOL/EOS vía la API pública de ciclo de vida en la primera creación de cada versión de SO, y un barrido de cierre que marca `RESUELTO` cualquier vulnerabilidad `source='redhat-lightspeed'` que ya no aparezca en el lote — posible solo porque esta fuente da una foto completa del CI, a diferencia de un informe manual. Ver `docs/INTEGRATIONS.md` § 9.13 para el detalle completo.
