...
Reescritura de `vcenterService.ts` para dejar de depender de `vcenter_sync`: la valla de retiro de VMs huérfanas pasa de `ciType=VCENTER_CI_TYPE AND vcenter_sync IS NOT NULL` a `ciType=VCENTER_CI_TYPE AND hypervisorId = <id resuelto de la fila VMWARE>` — igualdad exacta contra el id concreto del hipervisor que este conector posee, resuelto una vez al inicio de cada corrida de sync (no por VM). `hypervisorId` se asigna solo en la creación del CI (nunca en updates); `powerState` se refresca en cada update, reemplazando `vcenter_sync.powerState`. `esxiHost`/`cluster`/`lastSyncAt` por-VM se dejan de capturar en este punto (moref ya vive en `apiSlug`; esxiHost/cluster son `null` por el gap documentado en Task B; `lastSyncAt` es redundante con `CI.updatedAt`) — **nota:** la captura de `esxiHost` se retoma más adelante, best-effort, en la Task H2; `cluster`/`lastSyncAt` por-VM siguen sin capturarse. Con este cambio, un futuro conector OLVM sembraría su propia fila `Hypervisor` y usaría la misma receta de igualdad exacta contra su propio id — sin volver a depender de un chequeo de "no nulo" compartido.

## Task H1 — `24cef18` `feat(integrations): adopt pre-existing manually-entered CIs on first vCenter sync (match by name)`
El conector solo matcheaba VMs a CIs por `apiSlug === "vm-{moref}"`, lo que nunca reconoce los 208 CIs `VIRTUAL_SERVER` pre-existentes, entrados manualmente antes de que este conector existiera (su `apiSlug` es el que le puso un admin o una importación). Sin este fix, el primer sync habría creado un CI duplicado por cada VM ya inventariada. Cuando no hay match por `apiSlug`, ahora se busca un CI existente **sin clasificar** (`hypervisorId IS NULL`) cuyo nombre coincida (case-insensitive) con el de la VM descubierta; si el match es inequívoco (exactamente uno), se "adopta": se actualizan los campos físicos como de costumbre, y además se fija `apiSlug` al valor canónico `vm-{moref}` y `hypervisorId` al del hipervisor VMware — excepción de una sola vez a la regla D5 de que `hypervisorId` es "solo en la creación" — de modo que los syncs futuros lo reconocen directamente por `apiSlug` y esta rama nunca se vuelve a ejecutar para ese CI. Cero o dos-o-más candidatos caen al camino normal de creación de un CI nuevo; nunca se adivina a qué registro fusionar. `hypervisorId: null` es la valla de seguridad — la misma propiedad ya probada para la valla de retiro (Task G4): un CI ya clasificado por cualquier hipervisor nunca es tocado aquí.

## Task H2 — `7570d62` `feat(integrations): resolve ESXi host and create best-effort HOSTS relation on vCenter sync`
Resuelve el host ESXi en el que corre cada VM sincronizada (`VCenterClient.hostSummary()` + `VCenterConnector.discover()`, según los esquemas vSphere `VM.Summary`/`Host.Info` — no verificados independientemente contra un vCenter real en esta sesión, envueltos defensivamente para que cualquier suposición incorrecta degrade con seguridad al `esxiHost: null` de hoy) y, cuando existe exactamente un CI `PHYSICAL_SERVER` que coincide, crea una relación `CIRelation` `HOSTS` idempotente hacia él en `vcenterService.ts`. La resolución de `cluster` sigue fuera de alcance/`null`. El paso de la relación está aislado en su propio try/catch: un fallo ahí (ambigüedad, error de BD) nunca cuenta como error del sync de la VM ni bloquea la creación/actualización de su CI — es enriquecimiento best-effort, no un paso obligatorio.
