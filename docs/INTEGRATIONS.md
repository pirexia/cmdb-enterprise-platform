...
Además, `hypervisorId` es ahora **obligatorio** (validado en servidor y cliente) al crear/editar cualquier CI de tipo `VIRTUAL_SERVER` — se muestra como un desplegable "Hipervisor" en `AddCIModal`/`EditCIModal`, poblado desde `/api/masters/hypervisors`. `esxiHost`/`cluster`/`lastSyncAt` por VM se descartaron como captura (simplificación aceptada: el moref ya vive en `apiSlug`; esxiHost/cluster siempre eran `null` — gap ya documentado en el plan original; lastSyncAt es redundante con `CI.updatedAt`).
