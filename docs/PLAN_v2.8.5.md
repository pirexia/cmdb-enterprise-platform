...
**Migración idempotente** (`IF NOT EXISTS` / `ON CONFLICT DO NOTHING`) que siembra:
- CIType **"Sistema"** bajo la categoría adecuada (revisar `ci_type_categories` existentes)
- **NO** crear DateType `decommission-date` — ya existe en migración `20260614100000_date_types`

**Archivos:**
- `backend/prisma/migrations/<timestamp>_add_ci_type_sistema/migration.sql`
- `backend/prisma/schema.prisma` (si hay algún enum/cambio requerido)

**Skills:** `prisma-development`, `supabase-postgres-best-practices`
**DoD:** CIType "Sistema" disponible en inventario, migración idempotente.
**Commits estimados:** 1-2
**Commits realizados:** —
**PR:** —
