...
function FootprintCell({ fp }: { fp: Footprint }) {
  const kindColor: Record<string, string> = {
    RACK_SLOT:      "bg-green-100 border-green-400 text-green-700",
    INFRASTRUCTURE: "bg-slate-200 border-slate-400 text-slate-600",
    EMPTY:          "bg-white border-slate-200 text-slate-400",
  };
  const cls = kindColor[fp.kind] ?? "bg-white border-slate-200";
  return (
    <div
      title={fp.rackCi ? fp.rackCi.name : fp.label}
      className={`flex h-12 w-12 items-center justify-center border text-xs font-medium ${cls} ${!fp.active ? "opacity-40" : ""}`}
    >
      {fp.kind === "RACK_SLOT" && fp.rackCi ? (
...
export default function RoomPage() {
    ...
    try {
      await apiFetch(`/api/dcim/rooms/${id}`, {
        ...
        body   : JSON.stringify({
          name    : editName,
          kind    : editKind,
          widthMm : editWidth ? parseInt(editWidth) : null,
          depthMm : editDepth ? parseInt(editDepth) : null,
        }),
      });
      setEditing(false);
      await load();
  ...
  if (room) {
    room.footprints.forEach((f) => { fpMap[`${f.gridX},${f.gridY}`] = f; });
  }
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-wrap gap-4">
        <div className="flex items-center gap-2 rounded-none border border-slate-200 bg-white px-4 py-2 shadow-sm">
          <LayoutGrid className="h-4 w-4 text-slate-400" />
          <span className="text-sm text-slate-700">{room.footprints.length} {t("dcim.room.footprints")}</span>
        </div>
        <div className="flex items-center gap-2 rounded-none border border-slate-200 bg-white px-4 py-2 shadow-sm">
          <Server className="h-4 w-4 text-green-500" />
          <span className="text-sm text-slate-700">{racksAssigned} racks asignados</span>
        </div>
        <div className="flex items-center gap-2 rounded-none border border-slate-200 bg-white px-4 py-2 shadow-sm">
          <Zap className="h-4 w-4 text-amber-500" />
          <span className="text-sm text-slate-700">{room.aisles.length} pasillos</span>
        </div>
      </div>

      {/* 2D Plan placeholder / grid */}
        ...
        {room.footprints.length === 0 ? (
          ...
          <div className="overflow-auto p-4">
            ...
            <div className="mt-4 flex flex-wrap gap-4 text-xs text-slate-500">
              <span className="flex items-center gap-1"><span className="h-3 w-3 border border-green-400 bg-green-100 inline-block" /> Rack slot</span>
              <span className="flex items-center gap-1"><span className="h-3 w-3 border border-slate-400 bg-slate-200 inline-block" /> Infra</span>
              <span className="flex items-center gap-1"><span className="h-3 w-3 border border-slate-200 bg-white inline-block" /> Vacío</span>
            </div>
          </div>
        )}
      </div>
