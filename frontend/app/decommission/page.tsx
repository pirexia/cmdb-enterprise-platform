    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">{t("decommission.title")}</h1>
            <p className="text-sm text-slate-400">{t("decommission.subtitle")}</p>
          </div>
        </div>
        <div className="flex gap-2">
          <button onClick={fetchPlans} className="flex items-center gap-1.5 px-3 py-2 text-sm bg-slate-100 text-slate-600 rounded hover:bg-slate-200 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2 justify-end">
                      <button onClick={() => router.push(`/decommission/${p.id}`)}
                        className="flex items-center gap-1 px-3 py-1.5 text-xs bg-slate-100 text-slate-600 rounded hover:bg-slate-200 transition-colors">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                ...
                <input type="text" required value={newName} onChange={e => setNewName(e.target.value)}
                  className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 focus:outline-none focus:border-[var(--accent)]"
              ...
              <div>
                ...
                {systemsOpen && (
                  <div className="mt-1 rounded-lg border border-slate-200 bg-white divide-y divide-slate-100 max-h-40 overflow-y-auto">
                    {systemsLoading ? (
                      ...
                      ciOptions.map(ci => (
                        <button key={ci.id} type="button"
                          ...
                          className={`w-full text-left px-3 py-2 text-sm hover:bg-slate-50 transition-colors ${newCiId === ci.id ? "text-red-600 font-medium" : "text-slate-700"}`}>
                          {ci.name}
                        </button>
                      ))
                    )}
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => { setShowCreate(false); setNewName(""); setNewCiId(""); setNewCiSearch(""); setSystemsOpen(false); setCiOptions([]); }}
                  className="flex-1 px-4 py-2 text-sm bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200 transition-colors">
                  {t("actions.cancel")}
                </button>
