...
export default function InventoryPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const res = await apiFetch("/api/cis/bulk-delete", {
        method: "POST",
        body: JSON.stringify({ ciIds: Array.from(selectedIds) }),
      });
  ...
  return (
    <>
      ...
      {showBulkDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => !bulkDeleteLoading && setShowBulkDelete(false)}>
          <div className="w-full max-w-md rounded-none bg-white shadow-xl ring-1 ring-slate-200" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-slate-200 px-5 py-3">
              ...
              <button onClick={() => setShowBulkDelete(false)} disabled={bulkDeleteLoading}
            ...
            <div className="flex items-center justify-end gap-2 border-t border-slate-200 px-5 py-3 bg-slate-50">
              ...
              <button onClick={handleBulkDelete} disabled={bulkDeleteLoading}
                ...
                {t('inventory.bulk_delete.confirm_button')}
              </button>
            </div>
          </div>
        </div>
