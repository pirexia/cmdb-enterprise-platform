...
function SecurityBadge({ vulns }: { vulns: Vulnerability[] | null }) {
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-50 text-left">
                      ...
                      <th className="px-6 py-3 text-xs font-semibold uppercase tracking-wider text-slate-500">
                        <div className="flex items-center gap-1.5"><Shield className="h-3.5 w-3.5" />Seguridad</div>
                      </th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filtered.length === 0 ? (
                      <tr><td colSpan={6} className="py-12 text-center text-slate-400 text-sm">No se encontraron CIs.</td></tr>
                    ) : (
                      filtered.map((ci) => {
                          ...
                          <tr key={ci.id} className="group hover:bg-indigo-50/40 transition-colors">
                            ...
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                <SecurityBadge vulns={ci.vulnerabilities} />
                                {isAdmin && (
                                  <button onClick={() => handleScan(ci.id)} disabled={isScanning}
                                    className="flex items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-indigo-50 hover:border-indigo-300 hover:text-indigo-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
